...
\]
  ...
  because $x_1$ and $y_2$ both received their top
  ...
  stable. Recall that $\mathcal{R}$ is ordered, where $\{x_1, y_1, x_2, y_2\}$
...
\begin{proof}
  ...
  We can analyze the runtime of this algorithm. The \texttt{while} loop executes at
  ...
  |\mathcal{R}|$, where in practical cases there will be significantly fewer
  class sections than there are students. The significant
  factor for this bound is then $|\mathcal{R}|$. Therefore, the trading stage of the algorithm
  will be cubic in the size of $\mathcal{R}$.
  ...
  be trivial to have more seats in each class than there are students, so we cap
  the total number of seats to be that each class can have at most one seat per
  student. That means $|\mathcal{S}| \leq |\mathcal{C}| \times |\mathcal{R}|$.  So,
  our total runtime for the algorithm is $O((|\mathcal{C}||\mathcal{R}|+
  |\mathcal{R}|)^3 + (|\mathcal{C}| \times |\mathcal{R}|))$ =
  O$((|\mathcal{C}||\mathcal{R}|+ |\mathcal{R}|)^3)$. Therefore, the
