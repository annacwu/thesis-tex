...
first-year class registration. The registration software, SOLAR, opens to incoming
first-years at a certain time, and at that time (or later) all first-years log on to
...
the time of evaluation.  Here, ``available” considers the group caps on the seats from Definition
\ref{def:group-caps} in addition to overall capacity. So, a class that is not yet at
capacity is still unavailable to a student of group $T$ if it has already
reached its maximum number of group $T$ students, and filled all its free seats.
We will use ``available" in Algorithm \ref{alg:greedy} to mean the class is below
capacity, and has enough $T$-seats or free seats left to match student $t \in T$.
Essentially, \textsc{greedy} implements \textsc{fcfs} under the additional group
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1] \For{each student $t$ in $\mathcal{R}$} \Comment{where $t$ is a
    ...
    \If{$C$ is available to $t$} \State $\mu(t) \gets C$
...
\begin{proof}
  Observe that in a vanilla environment, $q^T = q = |F|$ because there are
  no group caps so every seat is essentially a free seat. So, ``available" in
  line 3 of Algorithm \ref{alg:greedy} will always be true when the class is
  below capacity $q$. Therefore,
...
\begin{theorem}
  ...
  general environment, even when a fair, inter-group stable matching exists.
\end{theorem}
...
\begin{proof}
  ...
  group. Let $\mathcal{R} = \{x_1, y_1, x_2, y_2, y_3\}$ be ordered where students $x_1, x_2 \in X$ and
  $y_1, y_2, y_3 \in Y$. Let the preference lists of the students be as follows: \[
  ...
  Observe that a fair, inter-group stable matching $\mu'$ does exist in this
  environment: \[ 
    \begin{array}{ll}
      x_1 &\gets C_1 \\ 
      y_1 &\gets C_3 \\
...
\]
  ...
  $\mu'$ is fair because only one student of each group is matched to each class,
  so seat reservations are respected. Additionally, we know $\mu'$ is stable
  because $x_1$, $y_2$, and $y_3$ all received their top
  choice, so none of them wish to swap with another student. $y_1$
  ...
  also want to swap with. $x_2$ wants to swap with $x_1$ or $y_3$, who we
  already know both do not wish to swap with anyone. Therefore, $\mu'$ does not have any
  blocking pairs, and thus it is inter-group stable.

  Now let $\mu$ denote \textsc{greedy}'s outputted matching over $\mathcal{R}$.
  We will prove by counter example that $\mu$ is not
  stable. Recall that $\mathcal{R}$ is ordered, where $\{x_1, y_1, x_2, y_2, y_3\}$
  is the order \textsc{greedy} matches the students. Observe that $\mu(x_1) =
  s_{C_1}^X$ and $\mu(y_1) = s_{C_2}^Y$:
  ...
  $C_2$ already has a student in group $Y$. $y_3$ remains unmatched, as there
  are no more seats in $C_1$. Notice that $\mu(y_2) >_{x_2}
  ...
  therefore \textsc{greedy} cannot guarantee a fair, inter-group stable matching
  under group cap constraints, even when a fair, inter-group stable matching exists.
\end{proof}
...
(and similar for $y_1$, $y_2$, and $y_3$). In fact, we claim that \textsc{greedy} will
always output an intra-group stable matching, as each student of each group is
matched to their top available choice at their point of evaluation.

\begin{theorem} \label{thm:greedy-intra}
  \textsc{greedy} will always output an intra-group stable matching if one
  exists. 
\end{theorem}

\begin{proof}
  Let $\mu$ denote the outputted matching over an ordered set of
  students, $\mathcal{R}$. Let $t_1, t_2 \in T \subset \mathcal{R}$, where $t_1$ comes
  before $t_2$ in the order.
  Assume for the sake of a contradiction that $t_1$ and $t_2$ form a blocking
  pair, meaning $\mu(t_2) >_{t_1} \mu(t_1)$ and $\mu(t_1) >_{t_2} \mu(t_2)$.

  By line 3 of Algorithm \ref{alg:greedy}, we know $\mu(t_1)$ was either $t_1$'s top
  available choice or $t_1$ is unmatched. So, if $\mu(t_2) >_{t_1} \mu(t_1)$, then
  $\mu(t_2)$ must not be available when $t_1$ is matched, which means there are
  no open seats in $T_s$ and no open free seats for that class. That means $t_2$ was
  matched first, to take the last $T_s$ or free seat. However, this is a
  contradiction of our assumption that $t_1$
  comes before $t_2$ in the input to the algorithm. Thus, $\mu(t_2)$ is available
  when $t_1$ is matched, so it must be the case that $\mu(t_1) >_{t_1} \mu(t_2)$.
  Therefore, $t_1$ and $t_2$ do not form a blocking pair, and we can conclude that
  $\mu$ is intra-group stable.
\end{proof}

While \textsc{greedy} is intra-group stable, its primary issue in the scope of
our project is that
it does not guarantee a full matching. Students can be left unmatched if every
class section on their preference list is already at capacity for their group,
as occurs with $y_3$ in the proof for Theorem \ref{thm:greedyunstable}.
...
stability to our maximum weight matching. To do that, we need to
remove all blocking pairs between students of the same group while preserving
the weight of the matching, as the weight is how we determined that
...
of various groups to fill the class. Notably by Theorem \ref{thm:full-weight},
it is possible to find a full maximum-weight matching that does
not fill every reserved seat; however, using the bound from Theorem
\ref{max-weight} was an effective way to mathematically ensure we are abiding by
group fairness. So, if we do not have a matching with weight  $|\mathcal{R}| +
|\mathcal{S}_{res}|$, we ``relax" seat reservations by decrementing the number of
...
\begin{proof}
  ...
  prefers their new match, so the edge retains weight at least 1
...
similar assignment problems that consider both inter-group stability and
...
\begin{theorem}
  \label{thm:polywt}
  The \textsc{wt} algorithm will output an intra-group stable matching in
  polynomial time with respect to the number of seats and students.
\end{theorem}
...
\begin{proof}
  ...
  the \texttt{while} loop (line 11) can execute per \texttt{for} loop (line 10) is as follows: \[
  ...
  Note that the sets of student groups are disjoint, so performing the loop for
  each group of students will still sum to a total of all students in
  $\mathcal{R}$. 

  We can analyze the runtime of this algorithm. As stated above, the \texttt{while} loop on line
  11 executes at
  most $|\mathcal{C}| \times |\mathcal{R}| \times \# \text{ groups}$ times,
  though we know we will only encounter each student as part of a disjoint group
  set, so that will still sum to an overall execution of $|\mathcal{C}| \times
  |\mathcal{R}|$. We therefore bound the execution of the \texttt{while} loop to
  $O(|\mathcal{C}| \times |\mathcal{R}|)$. Notably, $|\mathcal{C}| \leq
  |\mathcal{R}|$, and in practical cases there will be significantly fewer
  class sections than there are students. The significant
  factor for this bound is then $|\mathcal{R}|$. 

  Observe that before that, within the \texttt{while} loop on line 2, we run
  \textsc{max-weight} at least once, which runs in $O(V^3)$
  ...
  $(|\mathcal{S}| + |\mathcal{R}|)^3$. By construction of the problem, it would
  be trivial to have more seats in each class than there are students, so we
  impose a cap on
  the total number of seats where each class can have at most one seat per
  student. That means $|\mathcal{S}| \leq |\mathcal{C}| \times |\mathcal{R}|$,
  so \textsc{max-weight} will run in $(|\mathcal{C}||\mathcal{R}|+
  |\mathcal{R}|)^3$ time each iteration. The number of times \textsc{max-weight}
  can be called is bounded by the greatest number of reserved seats for the
  smallest group. That is, $\frac{q}{\# groups}$, or the class capacity
  divided by the number of groups, as that is the largest number of seats that the
  smallest group can have. So,
  our total runtime for the algorithm is \[O((\frac{q}{\# groups}\times(|\mathcal{C}||\mathcal{R}|+
  |\mathcal{R}|)^3) + (|\mathcal{C}| \times |\mathcal{R}|)) =
O(\frac{q}{\# groups}\times(|\mathcal{C}||\mathcal{R}|)^3)\]. Therefore, the
  \textsc{wt} algorithm outputs an intra-group stable matching in polynomial time with respect to
...
full, as every student is matched to a class by \textsc{max-weight} if such a
matching is possible (Theorem \ref{thm:full-weight}). Thus, the \textsc{wt}
algorithm we've developed
outputs an ISF1M solution to the HUM-assignment problem when such a solution exists.
The next step is to evaluate \textsc{wt} against \textsc{fcfs} and \textsc{greedy} to
