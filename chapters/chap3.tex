...
\begin{algorithm} \label{alg:fcfs}
  ...
  \begin{algorithmic}[1] 
     ...
     corresponding to registration order}
...
\begin{algorithm} \label{alg:greedy}
  ...
  \begin{algorithmic}[1] \For{each student $t$ in $R$} \Comment{where $t$ is a
    student of some group $T \subset R$} \For{each class $c$ in $P_t$} \If{\# students assigned to $c
      ...
      s_c^T$ \State \textbf{break}  \Comment{move to next student} \EndIf \EndFor  \State append $t$ to $X$ \EndFor

    \For{each student $t$ in $X$} \Comment{randomly assign unmatched
    students} \State choose
    random available $s_c$ from $C$ \State $\mu(t) \gets s_c$ \EndFor
...
algorithm starting on line 10 assigns remaining students randomly instead of leaving them
...
\begin{proof}
  ...
  as line 3 of \textsc{fcfs}. Now, let $\mu$ be the matching output by \textsc{fcfs}
  and $\mu'$ be the matching output by \textsc{greedy}. Any student not in $\mu$
  was matched to $\emptyset$. By Definition \ref{def:overview}, a student $r$ in
  $\mu'$ who received a random $s_c$ in lines 11 and 12 of \textsc{greedy} ranks $\emptyset \succ_r
  \mu'(r)$. Therefore, when evaluating blocking pairs, unmatched students in
  $\mu$ and randomly matched students in $\mu'$ will be functionally equivalent in
  terms of the rank of their assignment. Thus, \textsc{fcfs} is equivalent to
...
\begin{proof}
  ...
  By lines 3 and 11 of \textsc{greedy}, we know $\mu(r_1)$ was either $r_1$'s top
  available choice or a random assignment. So, if $\mu(r_2) >_{r_1} \mu(r_1)$, then
...
solution and we simply use the max-weight output to get a full matching for
