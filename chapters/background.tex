...
for an NP-complete problem, it would mean that every NP problem could be solved
...
\node[vertex] (2) at (2, 2.5) {2};
\node[vertex] (3) at (2, 0)   {3};
\node[vertex] (4) at (3.5, 1) {4};
\node[vertex] (5) at (5.5, 3) {5};
\node[vertex] (6) at (6.5, 0) {6};
...
\cite{graphs}. \setlength{\belowcaptionskip}{-10pt} \begin{figure}[H] \centering
  \begin{tikzpicture}[
    vertex/.style={circle, draw, thick, minimum size=8mm, inner sep=0pt, fill=#1},
    vertex/.default=blue!20
]

\node[vertex=orange!30]    (1) at (0, 2)   {1};
\node[vertex=green!30]   (2) at (2, 2.5) {2};
\node[vertex=orange!30]  (3) at (2, 0)   {3};
\node[vertex=blue!30] (4) at (3.5, 1) {4};
\node[vertex=green!30] (5) at (5.5, 3) {5};
\node[vertex=orange!30] (6) at (6.5, 0) {6};
...
Figure 1.1 colored with 3 colors} \label{fig:graph-color} \end{figure}
\setlength{\belowcaptionskip}{12pt}
...
\end{tikzpicture}\caption{Graph from
Figure 1.1 with weights } \label{fig:weighted-graph} \end{figure}
...
\node at (1, 1.2) {\textbf{men}};
\node at (5, 1.2) {\textbf{women}};
...
The difference between two-sided matching and fair division (also known as fair
...
the optimal solution: if an optimal solution has some value (such as a stable
...
guarantees a maximum of one blocking pair). Another optimization approach is
with \textit{heuristic} algorithms, which leans even more into the “good enough”
