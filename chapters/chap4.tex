...
Because of the level of randomness in our preference synthesis, in that Mallows
...
Recall our original goal for this project: assign students to classes under group fairness constraints. So, we evaluate how well each algorithm accomplishes this task. We can measure group fairness by looking at minimums and maximums: for each trial, what is the minimum number of students of each type in each section and what is the maximum? These metrics will indicate to us how well they are distributed. We can also look at the standard deviation of the number of students in each section for each group. 

We also are interested in stability, which means we want to check how many
intergroup and intragroup blocking pairs there are for each algorithm.
Relatedly, we calculate the average rank each student gave their assigned
section and we check how many students remain unmatched in the final matching. 

Finally, though it was not part of the design of the algorithms, we are interested in how well the sections are balanced regardless of student groups. Generally, we would hope all the sections are similarly sized. We can analyze section sizes through minimums, maximums, and the standard deviation as well. 
