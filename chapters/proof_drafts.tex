...
\begin{algorithm}
  ...
  \begin{algorithmic}[1] \For{each student $s$ in $S$} \Comment{start of round
      1} \For{each class $c$ in $C_s$} \If{# students assigned to $c$ < $q_c$} \State $\mu(s) \gets
...
\begin{proof}
  ...
  We can analyze the runtime of this algorithm. If the while loop executes at
  ...
  section of the algorithm will run in at most cubic time. Note that before
  that, we run the Max-Weight algorithm at least once, which runs in $O(V^3)$
  time where $V$ is the set of vertices [cite hungarian alg]. So, our total runtime for the algorithm
  is $O(V^3 + S^3)$.
  Therefore, the WT algorithm outputs a stable matching in cubic time.
\end{proof}
