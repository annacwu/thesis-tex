...
\begin{algorithm}  \caption{Weighted-Trading Algorithm \textit{WT}}
  \label{WT-alg} \begin{algorithmic}[1] \State $w(\mu) \gets 0$ \While{$w(\mu)
...
\begin{proof} At line 5 of Algorithm \ref{WT-alg}, $w(\mu) = |S| + |C_r|$
  because it was just output by the max-weight algorithm. So, we have to
  evaluate the for loop that compares each student of some type to each other
  student of that same type. In the for loop, the students swap seats if they
...
type, we replaced them with a student of the same type, so the edge retains the
additional weight of the reserved seat. Therefore, swapping any two students of
the same type does not change the weight of $\mu$. \end{proof}

\newpage \begin{theorem} The WT algorithm will output a stable matching.
\end{theorem}
...
we terminate this loop for each type of student $A \subset S$, then we must have
intra-group stability because no more blocking pairs remain in each group.
Therefore, the WT algorithm outputs a stable matching. \end{proof}
