...
\begin{algorithm} \caption{Greedy Algorithm (\textit{GA})}
  ...
  \begin{theorem} \textit{GA} outputs a stable matching in a vanilla
  environment. \end{theorem}

  \begin{proof} Let $\mu$ denote the outputted matching over an ordered set of
  ...
  $\mu$ is stable. \end{proof}

  \begin{theorem} \textit{GA} does not always output a stable matching with
  quotas (caps). \end{theorem}

  \begin{proof} Let $\mu$ denote \textit{GA}'s outputted matching over a set of
    ...
    students be as follows: \[ \begin{array}{ll} A1: & c_1 \succ c_3 \\ B1: &
  ...
  because $c_1$ already has a student of type $\alpha$ so $c_1$ is no longer
  available when matching $c_\alpha$, and similarly $\mu(B2) = c_1$ because
  $c_2$ already has a student of type $\beta$. Notice that $\mu(B2) >_A2
...
stable matching under quota/cap constraints. \end{proof}

\begin{theorem} There is not always a stable matching with quotas (caps) and
incomplete preferences. \end{theorem}

\begin{proof} Take the above instance, and note that if we alter the environment
  slightly there is no stable matching. Remove $c_3$ from $C$ so that $C = \{
  c_1, c_2\}$ and let the preference lists be as follows: \[ \begin{array}{ll}
  ...
  reserved evenly among each type of student, with an extra group of seats that
  ...
  seat $c$ is reserved for $s$'s type}\} + \mathbf{1}\{\text{if student $s$
prefers class $c$}\}. \]  \end{proof}

\begin{theorem} If a matching that respects student preferences and seat
...
the number of reserved seats per class and re-run the max-weight algorithm until
...
This means that for any type of student, that student does not form a blocking
pair with another student of the same type. For now, inter-group instability, or
blocking pairs involving students of different types, does not interfere with
our notion of stability.

Formally, a matching $\mu$ is stable if, for any two students $a_1$ and $a_2$ of
type $A \subset S$, \\ $\mu(a_1) >_{a_1} \mu(a_2)$  such that there exist no
intra-group blocking pairs. 
...
\begin{algorithm} \caption{Weighted-Trading Algorithm \textit{WT}}
  \begin{algorithmic}[1] \State $w(\mu) \gets 0$ \While{$w(\mu) \neq |S| +
    ...
    $|C_r|$} \EndWhile \State i = 1 \For{each student $a$ in $S_a$} \If{$\mu(a)
  <_a \mu(a_i)$ and $\mu(a_i) <_{a_i} \mu(a)$} \State $\mu(a) \gets \mu(a_i)$,
$\mu(a_i) \gets \mu(a)$ \EndIf \State i += 1\EndFor \State \Return matching
$\mu$ \end{algorithmic} \end{algorithm}

\begin{theorem} For a matching $\mu$ output by the WT algorithm, $w(\mu) = |S| +
|C_r|$. \end{theorem}

\begin{proof} At line 5 of the WT algorithm, $w(\mu) = |S| + |C_r|$ because it
  was just output by the max-weight algorithm. So, we have to evaluate the for
  loop that compares each student of some type to each other student of that
...
not change the weight of $\mu$. \end{proof}

\newpage

\begin{theorem} The WT algorithm will output a stable matching. \end{theorem}

\begin{proof} Let $\mu$ be the matching output by the Max-Weight algorithm at
